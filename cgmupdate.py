...
def read_spec_name(spec_path):
    sp = Specfile(spec_path,force_parse=True, macros=__macros)
    return sp.expanded_name
...
def process_spec(spec_path, components, update_mode):
    SOURCE_REGEX = re.compile(r"[^\s#]+")
    print(f"Processing: {spec_path}")

    sp = Specfile(spec_path,force_parse=True, macros=__macros)

    name = sp.expanded_name
    version = sp.expanded_version
    source_url = None
    with sp.sources() as sources:
        if len(sources) > 0:
            source_url = SOURCE_REGEX.search(sources[0].expanded_location).group()
...
##### for i in $(cm-uniq); do
##### cgmupdate last cgmanifest.json $i
##### done
